export default function drawLayer(props,layer)
{
  ...
  let exitCondition = this.hero.stats.bonuses[13]>=this.hero.coinsNeeded
  ...
  for (let c = startCol; c <= endCol; c++) {
    for (let r = startRow; r <= endRow; r++) {
      ...
      if (tile===200&&exitCondition) { // exit tile if conditions are met
        this.drawExit(layer,tile,x,y,this.ctx)
      } else if (tile>=100&&tile<=120) { // floor tile
        this.drawFloor(layer,tile,x,y,this.ctx)
      } else if (tile !== 0&&tile<40) { // 0 => empty tile
        this.drawBonus(layer,tile,x,y,this.ctx)
      } else if (tile>40&&tile<49) {  
        ...
        this.spawnEnemy({
          ...
          id:Math.floor(c*props.map.cols+c),
          ref:tile
        })
